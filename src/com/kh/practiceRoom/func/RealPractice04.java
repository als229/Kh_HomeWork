...
public class RealPractice04 {
	...
	public void practice1() {
		...
		while (true) {
			...
			if (!(ch == '+' || ch == '-' || ch == '*' || ch == '/' || ch == '%')) { // 이 연산자 이외의 것이 들어오면 잡아주는 조건식
			...
			} else if ((ch == '/' && num2 == 0) || (ch == '%' && num2 == 0)) { // 두번째 받은 정수의 값이 0이면서 ch 값이'/' 나 '%'일때
																				// 잡아주는 조건식
				System.out.println("0으로 나눌 수 없습니다. 다시 입력해 주세요");
				continue; // 컨티뉴로 다시 가장 가까운 반복문 시작으로 돌아감

			} else if (ch == '+' || ch == '-' || ch == '*' || ch == '/' || ch == '%') { // 연산자 값이 '+', '-', '*', '%',
																						// '/' 가 들어왔을때 돌아가는 조건식
				if (ch == '+') {
				...
				// System.out.println(num1 + " " + ch + " " + num2 + " = " + result ); // 이렇게 하면
				// '+', '-', '*'를 할때도 double 형으로 나온다

				if (ch == '/') { // printf에서 실수는 '%f'로 받기 때문에 따로 if로 출력문을 따로 만들어줌
					System.out.printf("%d %c %d = %f%n", num1, ch, num2, result);
				} else
					System.out.printf("%d %c %d = %d%n", num1, ch, num2, (int) result); // '/' 외에 나머지 결과값을 출력할 출력문
																						// double 에서 int는 강제형변환 가능!
			}
	...
	public void practice2() {
		...
		int num = sc.nextInt(); // 시작 숫자를 담을 startNum
		System.out.print("공차 : ");
...
//		}
		...
		String str = ""; // 초기값 num을 제외한 나머지 초기값을 담을 문자열 str 이라는 변수를 선언

		for (int i = 1; i <= 10; i++) {
			sum += gongCha;
			str += sum + (" "); // 이렇게 할 수도 있구낭 신기방기
		}
